...
from chainlit_literalai import LiteralDataLayer
from chainlit_literalai.data_layer import LiteralToChainlitConverter
...
@pytest.fixture
def literal_test_user(chainlit_test_user: User):
    return LiteralUser(
        id=str(uuid.uuid4()),
        created_at=datetime.datetime.now().isoformat(),
        identifier=chainlit_test_user.identifier,
        metadata=chainlit_test_user.metadata,
    )
...
async def test_create_user_not_existing(
    literal_data_layer: LiteralDataLayer,
    mock_literal_client: Mock,
    chainlit_test_user: User,
    literal_test_user: LiteralUser,
):
    mock_literal_client.api.get_user.return_value = None
    mock_literal_client.api.create_user.return_value = literal_test_user

    persisted_user = await literal_data_layer.create_user(chainlit_test_user)

    mock_literal_client.api.create_user.assert_awaited_once_with(
        identifier=chainlit_test_user.identifier, metadata=chainlit_test_user.metadata
    )
...
async def test_create_user_update_existing(
    ...
    mock_literal_client.api.update_user.assert_awaited_once_with(
        id=literal_test_user.id, metadata=chainlit_test_user.metadata
    )
